...
test('POST /trainings 201 (admin)', async () => {
  const { status, body } = await request(app())
    .post(`${apiRoot}`)
    .send({ access_token: adminSession, name: 'test', thumbnail: 'test', patient: 'test', exercises: 'test' })
...
test('PUT /trainings/:id 200 (admin)', async () => {
  const { status, body } = await request(app())
...
test('PUT /trainings/:id 404 (admin)', async () => {
  const { status } = await request(app())
...
test('DELETE /trainings/:id 204 (admin)', async () => {
  const { status } = await request(app())
...
test('DELETE /trainings/:id 404 (admin)', async () => {
  const { status } = await request(app())
