...
let user1, user2, trainer, session1, session2, adminSession;
...
test('GET /users 200 (trainer)', async () => {
  const { status, body } = await request(app()).get(apiRoot).query({ access_token: adminSession });
  expect(status).toBe(200);
  expect(Array.isArray(body)).toBe(true);
});

test('GET /users?page=2&limit=1 200 (trainer)', async () => {
...
test('GET /users?q=user 200 (trainer)', async () => {
  const { status, body } = await request(app())
    .get(apiRoot)
...
test('GET /users?fields=name 200 (trainer)', async () => {
  const { status, body } = await request(app())
    .get(apiRoot)
...
test('GET /users 401 (user)', async () => {
  const { status } = await request(app()).get(apiRoot).query({ access_token: session1 });
...
test('POST /users 401 (trainer)', async () => {
...
test('PUT /users/:id 200 (trainer)', async () => {
  const { status, body } = await request(app())
    .put(`${apiRoot}/${user1.id}`)
...
test('PUT /users/:id 401', async () => {
  const { status } = await request(app()).put(`${apiRoot}/${user1.id}`).send({ name: 'test' });
  expect(status).toBe(401);
});

test('PUT /users/:id 404 (trainer)', async () => {
  const { status } = await request(app())
...
test('DELETE /users/:id 204 (trainer)', async () => {
  const { status } = await request(app())
...
test('DELETE /users/:id 404 (trainer)', async () => {
  const { status } = await request(app())
